...
% expanded form, perterm a coefficient, and then compute and display the roots
...
% 		n = 21; epsilon - 0; 		perterm

% NOTE: The computed roots of the unperterbed polynomial may show some errors,
...
% Determine a suitable vertical scale for the graph.
% For each of the functions plotted, find the maximum absolute value for 1 <= x <= n,
% and then multiply the maximum of those maxima by 1.1
...
maxy = 1.1 * max([maxy, paxypert, maxperturb]);

plot(x, y, x, ypert, '--', x, perturbation, ':', ...
		x, zeros(size(x)), '-', 1:n, zeroes(1,n), 'o')
